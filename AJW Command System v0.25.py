...
#config
version = "v0.25" #note to myself: modify this later on next versions
...
try:
    if path.is_file():
        read = open("ajwconfig.txt").read()
        errorinfo = read
    else:
        with open('ajwconfig.txt', 'w+') as f:
            f.write("errorinfo=true")#false = disable detailed error report, true=enable detailed error report
